...
require('lazy').setup({
  ...
  -- Use custom neo-tree with hidden files shown
...
-- Commenting lines with Ctrl + / in Normal mode
vim.keymap.set('n', '<C-/>', 'gcc', { desc = 'Toggle comment on current line' })
-- Visual mode — toogle comment on selected lines
vim.keymap.set('v', '<C-/>', 'gc', { desc = 'Toggle comment on selected lines' })

-- NOTE: Tabs switching:

-- switching between tabs
vim.keymap.set('n', '<A-h>', ':tabprev<CR>', { desc = 'Go to the past tab' })
vim.keymap.set('n', '<A-l>', ':tabnext<CR>', { desc = 'Go to the next tab' })

-- Alternative mapping without Alt:
vim.keymap.set('n', '<leader>bp', ':tabprev<CR>', { desc = '[B]uffer [P]revious tab' })
vim.keymap.set('n', '<leader>bn', ':tabnext<CR>', { desc = '[B]uffer [N]ext tab' })

-- Go to the specific tab
for i = 1, 9 do
  vim.keymap.set('n', '<A-' .. i .. '>', function()
    vim.cmd(tostring(i) .. 'tabnext')
  end, { desc = 'Go to the tab ' .. i })
end
